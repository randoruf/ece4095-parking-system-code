...
class Model2DRigidCarSmooth:
    ...
    def euler_integrate(self, x: np.ndarray, u: np.ndarray, h: float):
        """
        ...
        """
        return x + h * self.state_transition_equation(x, u)
    ...
    def state_transition_equation(self, x: np.ndarray, u: np.ndarray):
        ...
        theta_loc = self.speed * u[0] * np.tan(x[3]) / self.car_length
        steer_loc = u[1]    # the steer rate ()
        return np.array([x_loc, y_loc, theta_loc, steer_loc])

    def integrate(self, x: np.ndarray, u: np.ndarray, h: float):
        """
        x is the state (x, y, theta, steer_angle)
        u is the control input (v, steer), note that u[0] can only be [-1, 1].
        h is the simulation step size in ODE integration
        """
        return self.euler_integrate(x, u, h)
    ...
    def valid_state(self, x: np.ndarray) -> bool:
        ...
        """
        return 0 <= x[0] <= self.x_upper_bound and 0 <= x[1] <= self.y_upper_bound and -np.pi <= x[2] < np.pi and \
            self.steer_angle_lower_bound <= x[3] <= self.steer_angle_upper_bound
    ...
    def distance_metric(x1: np.ndarray, x2: np.ndarray):
        ...
        dphi = min(abs(x1[3] - x2[3]), 2 * np.pi - abs(x1[3] - x2[3]))

        weighted_dtheta = (40/np.pi) * dtheta
        weighted_dphi = (2/np.pi) * dphi

        return np.sqrt(
            dx * dx +
            dy * dy +
            weighted_dtheta * weighted_dtheta +
            weighted_dphi * weighted_dphi
        )


class Node:
    def __init__(self, x: float, y: float, theta: float, steer: float):
        """
...
class RRTnoGoal:
    ...
    def _rand_conf(self) -> Node:
        ...
        steer_rand = np.random.uniform(-np.pi/6, np.pi/6)
        return Node(x_rand[0], y_rand[0], theta_rand, steer_rand)
    ...
    def _nearest_neighbour(self, q_rand: Node) -> Node:
        ...
        for i in range(0, len(self.node_list)):
            x_rand = np.array([q_rand.x, q_rand.y, q_rand.theta, q_rand.steer])
            x_tmp = np.array([self.node_list[i].x, self.node_list[i].y,
                              self.node_list[i].theta,
                              self.node_list[i].steer]
                             )
    ...
    def _steer(self, q_near: Node, q_rand: Node) -> Node:
        ...
        """
        x_near = np.array([q_near.x, q_near.y, q_near.theta, q_near.steer])
        x_rand = np.array([q_rand.x, q_rand.y, q_rand.theta, q_rand.steer])
        ...
        for velocity_direct in [-1, 1]:                         # velocity direction
            for steer_direct in range(-1, 2):       # steering direction
                h = np.random.uniform(low=1, high=3)
                u = np.array([velocity_direct, steer_direct*self.car_model.steer_rate])
        ...
        # print("---------------------")
        return Node(nx_best[0], nx_best[1], nx_best[2], nx_best[3])
    ...
    def buildRRT(self):
        ...
        for i in range(self.num_iter):
            ...
            x_new = np.array([q_new.x, q_new.y, q_new.theta, q_new.steer])
...
if __name__ == "__main__":
    ...
    # build a RRT
    q_start = Node(bounding_box//2, bounding_box//2, np.deg2rad(90), 0)
    ...
    # draw the car
    tmp_car = Car(ax, "blue")
    tmp_car.update_state(q_start.x, q_start.y, q_start.theta, q_start.steer)
